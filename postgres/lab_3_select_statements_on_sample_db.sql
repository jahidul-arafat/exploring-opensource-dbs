...
-- Section_09: NULL operator
-- Create a table <contacts> with first_name, last_name, email, phone
-- At the time of recording the contact, you may not know the contact's phone number. To deal with define the phone column as a nullable colum 
-- and insert NULL into the <phone> cloumn when yo8u save the contact information
...
-- WHERE 
-- 	table_name ILIKE 'cont%';

SELECT * FROM contacts;

-- Find the contacts who doesn't have a phone number
-- Not correct query, because anything comparing to NULL will always return false
SELECT *
FROM contacts
WHERE phone = NULL; -- this will return nothing; so output is empty; to solve use <IS NULL>

-- Correct One: try <IN NULL>
SELECT *
FROM contacts
WHERE phone IS NULL;

-- List all contacts whose has phone numbers
SELECT *
FROM contacts
